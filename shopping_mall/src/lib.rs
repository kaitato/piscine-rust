...
pub fn biggest_store(mall: Mall) -> Store {
    ...
    let mut biggest_store: &Store = &Store{ name: "".to_string(), square_meters: 0, employees: vec![] };

    for floor in mall.floors.iter() {
        for store in floor.stores.iter() {
            if store.square_meters > biggest_area {
                biggest_area = store.square_meters;
                biggest_store = store;
            }
        }
    }
         
    Some(Store { name: biggest_store.name.clone(), square_meters: biggest_store.square_meters, employees: biggest_store.employees.clone() }).unwrap()
}
...
pub fn check_for_securities(mall: &mut Mall, guards: Vec<Guard>) {
    ...
    for guard in guards {
        ...
        if number_of_meters / 200 > number_of_guards.try_into().unwrap() {
            // mall.guards.push(guard);
            let new_guard = Guard::new(&guard.name, guard.age, guard.years_experience);
            mall.hire_guard(new_guard);
            number_of_guards += 1;
            println!("{}", number_of_guards);
        } else {
            break;
        }
    }
